...
class GatewayHandler {
  ...
  uploadPushData(pushData) {
    ...
    if (pushData.stat && Object.keys(pushData.stat).length) {
      ...
      //promiseList.push(this._mqClient.publish(config.mqClient_nc.topics.pubToServer, stat));
    }
    ...
    if (pushData.rxpk && Object.keys(pushData.rxpk).length) {
      pushData.rxpk.forEach((element) => {
        ...
        //promiseList.push(this._mqClient.publish(config.mqClient_nc.topics.pubToServer, rxpk));
      });
      if (joinReq.length > 0) {
