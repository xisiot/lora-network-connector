...
class JoinHandler {
  ...
  parser(phyPayloadJSON) {
    ...
    return this.DeviceInfo.readItem(query, attr)
      .then((res) => {
        return this.micVerification(MICfields, res.AppKey, phyPayloadJSON.mic, 1);
      })
      .then(() => {
  ...
  Rejoinparser(phyPayloadJSON){
    const MACPayload = this.RejoinReqParser(phyPayloadJSON.macPayload);
    const phyPayload = {
      MHDR: phyPayloadJSON.mhdrJSON,
      MACPayload,
      MIC: phyPayloadJSON.mic,
    ...
    return this.DeviceInfo.readItem(query, attr)
      .then((res) => {
        return this.micVerification(MICfields, res.AppKey, phyPayloadJSON.mic, 2);
      })
      .then(() => {
        return bluebird.resolve(phyPayload);
      });
  ...
  RejoinReqParser (MACPayload) {
    ...
    RejoinReqJSON.DevEUI = slice(MACPayload, consts.REJOINDEVEUI_OFFSET, consts.RJCOUNT0_OFFSET);
    RejoinReqJSON.DevNonce = slice(MACPayload, consts.RJCOUNT0_OFFSET);
    return RejoinReqJSON;
  }


  micVerification (requiredFields, key, receivedMIC, sign) {
    return new bluebird((resolve, reject) => {
      ...
      if(sign === 1){
        ...
        calculatedMIC = this.joinMICCalculator(requiredFields, key, 'Rejoinrequest');
      }
