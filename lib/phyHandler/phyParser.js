...
PHYParser.prototype.parser = function (phyPayloadRaw) {
  ...
  console.log(phyPayloadJSON.mhdrJSON.MType);
  if (consts.NS_MSG_TYPE_LIST.indexOf(phyPayloadJSON.mhdrJSON.MType) > -1) {
    ...
    return PHYParser.macPayloadParser(phyPayloadJSON.macPayload)
      .then((macPayloadJSON) => {
        ...
        const queryAttributes = ['FNwkSIntKey', 'AppSKey'];
        const queryOpts = {
            ...
            .then((res) => {
              if (!res.FNwkSIntKey) {
                ...
                return _this.DeviceInfoMysql.readItem(queryOpts, consts.DEVICEINFO_CACHE_ATTRIBUTES)
                  ...
                  .then(() => {
                    const res = {
                      FNwkSIntKey: devInfo.FNwkSIntKey,
        ...
        const macPayloadMICVerify = function (values) {
          ...
          const FNwkSIntKey = values.FNwkSIntKey;
          const micCal = phyUtils.micCalculator(requiredFields, FNwkSIntKey, direction);
  ...
  } else if ((consts.JS_MSG_TYPE_LIST.indexOf(phyPayloadJSON.mhdrJSON.MType) > -1) && (phyPayloadJSON.mhdrJSON.MType === 0 )) {
  ...
  } else if ((consts.JS_MSG_TYPE_LIST.indexOf(phyPayloadJSON.mhdrJSON.MType) > -1) && (phyPayloadJSON.mhdrJSON.MType === 6 )) {
    if ((phyPayloadJSON.macPayload.length !== consts.REJOINREQ_1_BASIC_LENGTH)||(phyPayloadJSON.macPayload.length !== consts.REJOINREQ_2_BASIC_LENGTH)) {
      return bluebird.reject(
        new ERROR.InvalidMessageError({
          message: `Invalid length of REJOIN request, ${consts.RE} bytes is mandatory`,
          receivedLength: phyPayloadJSON.macPayload.length,
        })
      );
    }
    return this.joinHandler.Rejoinparser(phyPayloadJSON);
  } else {
    return bluebird.reject(
