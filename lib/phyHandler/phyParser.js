            ...
            .then((res) => {
              if (!res.NwkSKey) {
                ...
                return _this.DeviceInfoMysql.readItem(queryOpts, consts.DEVICEINFO_CACHE_ATTRIBUTES)
                  .then((res) => {
                    if (!res.AppKey || !res.AppEUI) {
                      const errorMsg = {
                        DevAddr: macPayloadJSON.fhdrJSON.DevAddr,
                        msg: 'The Device was not registered in LoRa web',
                      };
                      return bluebird.reject(
                        new ERROR.DeviceNotExistError(errorMsg)
                      );
                    }
                    if (!res.NwkSKey || !res.AppSKey) {
                      const errorMsg = {
                        DevAddr: macPayloadJSON.fhdrJSON.DevAddr,
                        msg: 'The OTAA Device was not registered through the join process',
                      };
                      return bluebird.reject(
                        new ERROR.DeviceNotExistError(errorMsg)
                      );
